...
class PuestaDisposicionController extends Controller
{
    ...
    private function unidadIdDesdeActividad(?Actividad $actividad): ?int
    {
        if (!$actividad) return null;

        $unidadId=(int)($actividad->unidad_org_id ?: optional($actividad->creador)->unidad_id);
        return $unidadId>0 ? $unidadId : null;
    }
    ...
    private function resolverActividadOrigen(Request $request,$usuario): ?Actividad
    {
        $actividadId=(int)$request->input('actividad_id');
        if ($actividadId<=0) return null;

        $query=Actividad::query()
            ->with(['creador','vehiculos'])
            ->whereKey($actividadId);

        $this->applyActividadVisibilityScope($query,$usuario);
        $actividad=$query->first();

        if (!$actividad) {
            throw ValidationException::withMessages([
                'actividad_id'=>'No se encontró una actividad válida para vincular la puesta a disposición.',
            ]);
        }
    ...
    private function applyActividadVisibilityScope($query,$usuario): void
    {
        ...
        $query->where(function ($unidadQuery) use ($unidadId) {
            $unidadQuery->where('unidad_org_id',$unidadId)
                ->orWhere(function ($legacy) use ($unidadId) {
                    $legacy->whereNull('unidad_org_id')
                        ->whereHas('creador',fn ($creador) => $creador->where('unidad_id',$unidadId));
                });
        });

        if ($unidadId!==PuestaDisposicionRules::UNIDAD_DELEGACIONES_ID) {
            return;
        }

        if ($usuario->hasRole('Administrador') || $usuario->hasRole('Administrativo') || $usuario->hasRole('Subdirector')) {
            return;
        }

        $delegacionIds=HechoAccess::delegacionIdsVisiblesParaUsuario($usuario);
    ...
    private function resolverVehiculoRelacionadoId($origen,array $vehiculo): ?int
    {
        if (!$origen) return null;

        $origen->loadMissing('vehiculos');
        $vehiculos=$origen->vehiculos;
    ...
    private function guardarDetalles(PuestaDisposicion $puesta, Request $request, array &$documentPaths): void
    {
        $origen=$puesta->hecho_id
            ? $puesta->hecho()->with('vehiculos')->first()
            : ($puesta->actividad_id
                ? $puesta->actividad()->with('vehiculos')->first()
                : null);

        foreach ($request->input('personas', []) as $index => $persona) {
            if (empty(trim((string)($persona['nombre_completo'] ?? '')))) continue;

            $archivoUsoFuerzaPath=null;
            $archivoUsoFuerza=$request->file("personas.$index.archivo_uso_fuerza");
            if ($archivoUsoFuerza) {
                $archivoUsoFuerzaPath=$this->documentos()->putUploadedPdf(
                    $archivoUsoFuerza,
                    'puestas_disposicion/uso_fuerza/personas'
                );
                $documentPaths[]=$archivoUsoFuerzaPath;
            }
            ...
            PuestaDisposicionPersona::create([
                ...
                'archivo_uso_fuerza'=>$archivoUsoFuerzaPath,
            ]);
        }
        ...
        foreach ($request->input('vehiculos', []) as $vehiculo) {
            ...
            PuestaDisposicionVehiculo::create([
                'puesta_disposicion_id'=>$puesta->id,
                'vehiculo_id'=>$this->resolverVehiculoRelacionadoId($origen,$vehiculo),
    ...
    public function store(Request $request)
    {
        ...
        if (
            ((int)$request->input('actividad_id')>0)
            && (((int)$request->input('hecho_id')>0) || trim((string)$request->input('hecho_client_uuid',''))!=='')
        ) {
            throw ValidationException::withMessages([
                'actividad_id'=>'La puesta sólo puede vincularse con un hecho o con una actividad, no con ambos.',
            ]);
        }

        $hechoOrigen=$this->resolverHechoOrigen($request,$usuario);
        $actividadOrigen=$this->resolverActividadOrigen($request,$usuario);
        ...
        if ($actividadOrigen && PuestaDisposicion::query()->where('actividad_id',$actividadOrigen->id)->exists()) {
            throw ValidationException::withMessages([
                'actividad_id'=>'Esta actividad ya tiene una puesta a disposición vinculada.',
            ]);
        }

        $unidadRegistroId=$this->unidadIdDesdeHecho($hechoOrigen)
            ?: $this->unidadIdDesdeActividad($actividadOrigen)
            ?: $this->resolverUnidadRegistro($request,$usuario);
        $request->merge([
            'hecho_id'=>$hechoOrigen ? $hechoOrigen->id : null,
            'actividad_id'=>$actividadOrigen ? $actividadOrigen->id : null,
        ]);
        ...
        Log::info('Puesta a disposicion API: solicitud recibida', [
            ...
            'tiene_archivo_uso_fuerza'=>$request->hasFile('archivo_uso_fuerza'),
        ]);
        ...
        try {
            ...
            if ($request->hasFile('archivo_puesta')) {
                $archivo=$this->documentos()->putUploadedPdf($request->file('archivo_puesta'),'puestas_disposicion');
                $documentPaths[]=$archivo;
            }

            $archivoUsoFuerzaGeneral=null;
            if ($request->hasFile('archivo_uso_fuerza')) {
                $archivoUsoFuerzaGeneral=$this->documentos()->putUploadedPdf(
                    $request->file('archivo_uso_fuerza'),
                    'puestas_disposicion/uso_fuerza'
                );
                $documentPaths[]=$archivoUsoFuerzaGeneral;
            }
            ...
            $puesta=PuestaDisposicion::create([
                ...
                'unidad_id'=>$unidadRegistroId,
                'delegacion_id'=>$hechoOrigen
                    ? ($hechoOrigen->delegacion_id ?: $usuario->delegacion_id)
                    : ($actividadOrigen
                        ? ($actividadOrigen->delegacion_id ?: $usuario->delegacion_id)
                        : $usuario->delegacion_id),
                'destacamento_id'=>$actividadOrigen
                    ? ($actividadOrigen->destacamento_id ?: $usuario->destacamento_id)
            ...
            $this->guardarDetalles($puesta, $request, $documentPaths);
    ...
    public function archivoUsoFuerzaGeneral(PuestaDisposicion $puestaDisposicion)
    {
        ...
        abort_unless($puesta->archivo_uso_fuerza,404);

        $nombre='uso_fuerza_puesta_' . $puesta->numero_puesta . '_' . $puesta->anio . '.pdf';

        return $this->documentos()->response($puesta->archivo_uso_fuerza,$nombre);
    }
    ...
    public function archivoUsoFuerza(PuestaDisposicion $puestaDisposicion, PuestaDisposicionPersona $persona)
    {
        ...
        abort_unless((int)$persona->puesta_disposicion_id===(int)$puesta->id,404);
        abort_unless($persona->archivo_uso_fuerza,404);

        $nombre='uso_fuerza_persona_' . $persona->id . '_puesta_' . $puesta->numero_puesta . '_' . $puesta->anio . '.pdf';
    ...
    public function update(Request $request,PuestaDisposicion $puestaDisposicion)
    {
        ...
        $documentosNuevos=[];
        $archivoPuestaAnterior=$puesta->archivo_puesta;
        $archivoUsoFuerzaAnterior=$puesta->archivo_uso_fuerza;

        try {
            $archivoPuesta=$archivoPuestaAnterior;
            $archivoUsoFuerza=$archivoUsoFuerzaAnterior;
            ...
            foreach ([
                ...
                [$archivoUsoFuerzaAnterior,$archivoUsoFuerza],
            ] as [$anterior,$actual]) {
                if ($anterior && $anterior !== $actual) {
                    $this->documentos()->delete($anterior);
                }
            }
        } catch (\Throwable $e) {
        ...
        return response()->json($this->withFotoUrls($puesta->fresh()));
    }
    ...
    public function destroy(PuestaDisposicion $puestaDisposicion)
    {
        ...
        $puesta->loadMissing(['fotos','personas']);
        $archivo=$puesta->archivo_puesta;
        $archivoUsoFuerzaGeneral=$puesta->archivo_uso_fuerza;
        $archivosUsoFuerza=$puesta->personas->pluck('archivo_uso_fuerza')->filter()->all();
        ...
        if ($archivoUsoFuerzaGeneral) {
            $this->documentos()->delete($archivoUsoFuerzaGeneral);
        }
    ...
    private function withFotoUrls(PuestaDisposicion $puesta): array
    {
        $puesta->loadMissing(['fotos','personas']);
        $data=$puesta->toArray();
        $data['archivo_puesta_url']=$puesta->archivo_puesta
            ? route('api.puestas_disposicion.archivo',$puesta->id)
            : null;
        $data['archivo_uso_fuerza_url']=$puesta->archivo_uso_fuerza
            ? route('api.puestas_disposicion.uso_fuerza',$puesta->id)
            : null;
        $data['personas']=$puesta->personas->map(function (PuestaDisposicionPersona $persona) use ($puesta) {
            $item=$persona->toArray();
            $item['archivo_uso_fuerza_url']=$persona->archivo_uso_fuerza
                ? route('api.puestas_disposicion.personas.uso_fuerza',[$puesta->id,$persona->id])
                : null;
            return $item;
        })->values()->all();
