    ...
    <div class="sv-hero">
        <div class="sv-hero__inner">
            <div class="sv-hero__badge">
                ...
                <span>Siniestros · Parte de Novedades · Cortes</span>
            </div>
            ...
            <div class="sv-hero__subtitle">
                Listado de cortes disponibles para descarga
            </div>
        </div>
    </div>
@stop
        ...
        <div class="col-12">
            <div class="sv-panel">
                <div class="sv-panel__header">
                    <div>
                        <div class="sv-panel__title">Cortes generados</div>
                        <div class="sv-panel__desc">Selecciona una fecha para descargar el parte correspondiente.</div>
                    </div>
                </div>
                        ...
                        <table class="table sv-table mb-0">
                            ...
                            <tbody>
                                @foreach($cortes as $corte)
                                    <tr>
                                        <td>{{ $corte['fecha'] ?? '' }}</td>
                                        <td>{{ $corte['inicio_texto'] ?? '' }}</td>
                                        <td>{{ $corte['fin_texto'] ?? '' }}</td>
                                        <td>{{ $corte['hora_corte'] ?? '' }}</td>
                                        <td>
                                            <a href="{{ route('settings.estadisticas_siniestros.parte_novedades.descargar', $corte['fecha']) }}" class="btn sv-btn">
                ...
                @else
                    <div class="sv-empty">
                        ...
                        <div class="sv-empty__title">No hay cortes disponibles</div>
                        <div class="sv-empty__desc">Cuando existan cortes de parte de novedades aparecerán aquí.</div>
                    </div>
                @endif
            </div>
        </div>
...
@section('js')
    <script> console.log('Parte de Novedades'); </script>
@stop
