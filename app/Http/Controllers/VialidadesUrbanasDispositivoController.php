...
{
    public function index($vialidadUrbana, Request $request)
    {
    ...
    public function create($vialidadUrbana)
    {
    ...
    public function store($vialidadUrbana, Request $request)
    {
        $usuario = auth()->user();
        ...
        $dispositivo = VialidadDispositivo::create($data);
    ...
    public function show($vialidadUrbana, VialidadDispositivo $dispositivo)
    {
    ...
    public function edit($vialidadUrbana, VialidadDispositivo $dispositivo)
    {
        $this->authorizeDispositivo($dispositivo);
    ...
    public function update($vialidadUrbana, Request $request, VialidadDispositivo $dispositivo)
    {
        $this->authorizeDispositivo($dispositivo);
    ...
    public function destroy($vialidadUrbana, VialidadDispositivo $dispositivo)
    {
        $this->authorizeDispositivo($dispositivo);

        $dispositivo->load(['fotos']);
        ...
        return redirect()
            ->route('vialidades_urbanas.dispositivos.index', $vialidadUrbana)
            ->with('success', 'Dispositivo eliminado correctamente.');
    }

    public function whatsapp($vialidadUrbana, VialidadDispositivo $dispositivo)
    {
        $this->authorizeDispositivo($dispositivo);

        $dispositivo->load(['catalogo', 'detalles']);
        ...
        if (!empty($dispositivo->municipio)) {
            $lineas[] = '';
            $lineas[] = trim((string) $dispositivo->municipio);
        }

        $lineas[] = '';
        $lineas[] = strtoupper(\Carbon\Carbon::parse($dispositivo->fecha)->locale('es')->translatedFormat('l d F Y'));
        $lineas[] = strtoupper(substr((string) $dispositivo->hora, 0, 5) . ' HORAS');
        $lineas[] = '';
        ...
        if (!empty($dispositivo->supervision)) {
            $lineas[] = trim((string) $dispositivo->supervision);
        }
        ...
        if (!empty($dispositivo->responsable_cargo)) {
            $lineas[] = trim((string) $dispositivo->responsable_cargo);
        }

        if (!empty($dispositivo->responsable_nombre)) {
            $lineas[] = trim((string) $dispositivo->responsable_nombre);
        }
