...
class NovRelSheetService extends BaseSiniestrosSheetService
{
    public function generar(
        ...
        foreach ($actividades as $actividad) {
            $fotos = $this->rutasFotosActividad(
                $actividad,
                $fotosPorActividad
            );
        ...
        if ($numero === 1) {
            ...
            $sheet->setCellValue(
                'B' . $fila,
                'SIN NOVEDADES RELEVANTES EN EL PERIODO.'
            );
        }
    ...
    private function aplicarFormatoBase(
        Worksheet $sheet
    ): void {
    ...
    private function fotosPorActividad(
        Collection $actividades
    ): Collection {
        $ids = $actividades
            ->pluck('id')
    ...
    private function rutasFotosActividad(
        $actividad,
        Collection $fotosPorActividad
    ): array {
        $rutas = [];
    ...
    private function lugarActividad(
        $actividad
    ): string {
        foreach ([
    ...
    private function textoActividad(
        $actividad
    ): string {
        $texto = trim(
            implode(
        ...
        foreach ([
            ...
            $actividad->categoria_nombre ?? null,
        ] as $valor) {
            $valor = trim(
                (string) $valor
            );
        ...
        return 'ACTIVIDAD DE LA UNIDAD DE ATENCIÓN A SINIESTROS';
    }
    ...
    private function rutaCompatibleExcel(
        string $path
    ): ?string {
        $tipo = @exif_imagetype(
            $path
